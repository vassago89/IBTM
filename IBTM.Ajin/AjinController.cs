...
public sealed class AjinController(AjinSettings settings, ApplicationLog? log = null) : IDisposable
{
    ...
    public bool ReadRtexInput(int channel)
    {
        lock (_gate)
        {
            ...
            var value = 0U;
            Check(
                CAXD.AxdiReadInportBit(module, offset, ref value),
    ...
    public void ReadRtexInputs(uint[] values)
    {
        ...
        if (values.Length != RtexInputWordCount)
            throw new ArgumentException(
                $"Expected {RtexInputWordCount} AJIN input module slots.",
                nameof(values));
    ...
    public bool ReadRtexOutput(int channel)
    {
        lock (_gate)
        {
            ...
            var value = 0U;
            Check(
                CAXD.AxdoReadOutportBit(module, offset, ref value),
    ...
    public void WriteRtexOutput(int channel, bool value)
    {
        lock (_gate)
        {
            ...
            var (module, offset) = GetRtexAddress(_outputModules, _outputCounts, channel, "DO");
            Check(
                CAXD.AxdoWriteOutportBit(module, offset, value ? 1U : 0U),
    ...
    private static int[] GetChannelCounts(
        int[] modules,
        Dictionary<int, (int Inputs, int Outputs)> counts,
        bool input)
    {
        ...
        for (var index = 0; index < modules.Length; index++)
        {
            ...
            if (count is not (16 or 32))
                throw new IOException(
                    $"AJIN configured {(input ? "input" : "output")} module={modules[index]} has {count} {(input ? "DI" : "DO")} points; the RTEX mapping requires 16 or 32 points per module.");
            result[index] = count;
        }
    ...
    private static uint ReadInputWord(int module, int offset)
    {
        uint value = 0;
        Check(
            CAXD.AxdiReadInportWord(module, offset, ref value),
            nameof(CAXD.AxdiReadInportWord),
            module,
            offset);
        return value & 0xFFFF;
    }
    ...
    private void EnsureReady(string operation)
    {
        if (!_initialized)
            throw new IOException(
                $"{operation} cannot run: AJIN initialization and DIO module validation have not completed.");
    }
        ...
        string direction)
    {
        if (channel < 0 || channel / RtexChannelCountPerModule >= modules.Length)
            throw new ArgumentOutOfRangeException(
                nameof(channel),
                channel,
                "The RTEX channel is outside the configured module slots.");
        var index = channel / RtexChannelCountPerModule;
        var module = modules[index];
        var offset = channel % RtexChannelCountPerModule;
        if (offset >= counts[index])
            throw new IOException(
                $"AJIN RTEX {direction} channel={channel} maps to module={module}, offset={offset}, but the module has only {counts[index]} {direction} points.");
