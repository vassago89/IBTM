...
public partial class App : System.Windows.Application
{
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        try
        {
            ...
            _log.Write($"Connections: AlphaMotion TMC-AE16DIOe card={settings.AlphaMotion.ControllerNumber}, 16 DI / 16 DO; AJIN interrupt={settings.Ajin.InterruptNumber}, input modules=[{string.Join(",", settings.Ajin.RtexInputModules ?? [])}], output modules=[{string.Join(",", settings.Ajin.RtexOutputModules ?? [])}], motion file={settings.Ajin.MotionParameterFile}.");
        }
