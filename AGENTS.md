...
- Bind directly to existing observable state, nested properties and command properties instead of copying values or forwarding their change notifications through the view model. Use XAML triggers for simple display-only conditions; keep sequence decisions, real interlocks and nontrivial calculations in named C# members. Preserve null/unknown feedback in bindings.
...
- Use `_camelCase` for private instance fields and `s_camelCase` for private static fields, including `readonly` fields. Use `PascalCase` for constant fields, non-private fields, properties, methods and events; use `camelCase` for parameters and local variables. Record positional parameters follow their generated property names. Reserve a standalone `_` for discards and ignored lambda parameters; name method/event-handler parameters normally even when unused. Do not use `__`, `m_` or underscore-prefixed methods/properties. Apply renames to symbol references without changing serialized names or manufacturer SDK declarations; `.editorconfig` records the field/property rules.
...
- Do not extract a helper unless at least two call sites actually reuse it. Inline single-call helpers into their caller instead of splitting code just to shorten a method. Keep named command/event handlers and required interface/override entry points; preserve hardware boundaries, cancellation lifetimes, feedback handling and real interlocks. Remove redundant axis-specific wrappers when the implementation already takes an axis.
...
- Inject the shared `UnitSettings` instance for unit enablement instead of separate setting getter delegates. Read its current values where the sequence decision belongs; keep DI focused on wiring objects.
...
- Declare local variables where their first value is assigned, using `var` when the type is clear. Keep their scope narrow instead of collecting uninitialized declarations above a `try` or operation block.
...
- Use injected `Microsoft.Extensions.Logging.ILogger<T>` directly for logging. Let Serilog's file/async sinks own persistence and flushing; keep `ApplicationLog` for observable screen history and logging-failure feedback, without custom file queues or logging wrappers.
