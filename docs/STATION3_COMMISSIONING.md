...
- 정방향 이송은 목적지 **히트싱크 1 감지 뒤** 설정된 시간만큼 더 구동해 물리적 간격을 이동한다.
  히트싱크 2만 감지됐을 때는 이 타이머를 시작하지 않는다.
  `CarrierStopDelaySeconds`는 센서 도착 제한 시간이 아니다.
  밀착 중 STOP하면 START 후 설정 시간 전체를 다시 구동한다. 플레이트 하강·스토퍼 상승
  피드백이 필요하며, 히트싱크 1이 지나갔어도 2가 켜져 있으면 도착 이력을 유지한다.
  밀착 중 두 히트싱크 입력이 모두 꺼지면 정지하고 상승으로 넘어가지 않는다.
  밀착을 마치고 플레이트 상승 중 멈춘 경우에는 현재 착좌 피드백으로 상승을 이어간다.
...
메인 반환은 입구 전용 센서에서 멈춘다. 다음 전진도 Station 1의 HS1 감지 후 설정 시간만큼
...
- 감지 직후 밀착 중 STOP과 플레이트 상승 중 STOP을 나눠 확인한다. 전자는 밀착을 다시
  수행하고, 후자는 지지대를 내리거나 벨트를 다시 돌리지 않아야 한다. 밀착 중 멈춘 뒤 수동으로
  지지대를 올렸다면 재시작이 지지대를 자동으로 내리지 않고 밀착 조건 오류를 알리는지 확인한다.
