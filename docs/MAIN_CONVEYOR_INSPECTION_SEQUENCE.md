...
검사/NG 트랜스퍼의 전용 대기 위치와 S3 픽업 위치는 별도로 티칭한다.
...
| 검사 전 다른 캐리어 이송 | 픽업 위치 이동 완료 → 플레이트 상승 확인 → 스토퍼 하강 | S1 → S2, 전단 → S1 등 현재 가능한 이송을 먼저 수행 | 검사 시작 보류 |
| 우선 이송 종료 후 검사 준비 | 스토퍼 UP 확인 → 플레이트 DOWN 확인 | 정지 | 메인의 검사 요청을 기다림 |
| S3 검사 | DOWN / UP | 정지 유지. 다른 캐리어의 벨트 이송도 대기 | 바코드·볼트 검사 위치로 이동 |
| 검사 후 복귀 | DOWN / UP | 정지 유지 | 전용 대기 위치로 XY 동시 복귀 |
| OK, 후방 Ready ON | DOWN / 스토퍼 하강 | 바로 후방 배출 | 전용 대기 위치에서 대기 |
| OK, 후방 Ready OFF | 픽업 위치 이동 완료 → 플레이트 상승 확인 → 스토퍼 하강 | 다른 캐리어의 반입·S1 → S2 이송 가능 | 상승 처리 후 전용 대기 위치 복귀 |
| 상승 대기 중 후방 Ready ON | 플레이트 하강 | 하강부터 후방 배출까지 한 동작으로 수행 | 전용 대기 위치 확인 |
| NG | 픽업 위치 이동 완료 → 플레이트 상승 확인 → 스토퍼 하강 | 다른 캐리어의 반입·S1 → S2 이송 가능 | 상승된 S3 지지 확인 후 내려가 집고 NG 셔틀로 이송 |
| 빈 트랜스퍼의 대기 | 캐리어 유무·검사 결과에 따른 상태 유지 | 메인 컨베이어가 판단 | 전용 대기 위치로 돌아와 대기 |
...
S3 픽업 위치는 `CarrierPickupPosition`, 전용 대기 위치는 `WaitingPosition`이다.
각 항목에서 X/Y를 함께 티칭하며 XY 동시 이동한다.
메인은 `RequestCarrierSeating(job)`을 보내고 검사 유닛이
`SeatStationAsync`에서 픽업 위치 이동부터 S3 상승까지 한 동작으로 수행한다.
다른 루프가 현재 좌표만 비교하여 플레이트 상승을 허용하지 않는다.

## S3 캐리어의 세 가지 작업 단계

1. **검사 전 물류 대기** (`WaitingForConveyor`): S3를 올려 벨트에서 분리하고,
   지금 실행 가능한 다른 캐리어의 이송을 먼저 처리한다. 전단 반입도 포함한다.
2. **검사 진행** (`PreparingInspection` 및 바코드·볼트·복귀 세부 상태): 가능한 우선 이송이 끝나면
   S3를 내리고 메인이 해당 캐리어의 검사를 요청한다. 요청 이후 검사·복귀 완료까지 벨트를 정지한다.
3. **검사 완료 후 이송 대기**: 기존 OK 즉시 배출, OK 상승 대기,
   NG 상승 집기의 분기를 따른다.
...
반대로 검사 요청 이후 들어온 반입·이송 요청은 검사와 전용 대기 위치 복귀가 끝날 때까지 기다린다.
...
| `WaitingForInspection` | 검사와 전용 대기 위치 복귀가 끝나기를 기다린다. 메인 벨트는 정지한다. S1·S2의 플레이트 상승 및 개별 스테이션 작업은 가능하다. |
| `WaitingForInspectionTransfer` | 검사 갠트리의 동작 완료·픽업 상승·미완료 인계 해제 또는 전용 대기 위치 피드백을 기다린다. |
...
`Step`은 현재 실행·대기 표시이며 중단 후 재개 위치를 저장하지 않는다.
...
검사 위치는 `InspectionStation.IsAtInspectionPosition`으로 읽는다.
...
5. 일반 자동은 셔틀 인계 후 `WaitingForShuttleDown`에서 셔틀 DOWN을 확인하고 대기 위치로 복귀한다.

검사 중 플레이트·스토퍼·재실 조건이 깨지거나 벨트 Run 출력이 켜지면 진행 중 검사를 취소한다.
취소된 촬영 결과를 완료로 기록하지 않는다. 이미 수집한 결과의 캐리어별 소유권은 유지한다.
검사할 캐리어가 없거나 티칭을 기다리는 경우의 `ReturningToWaitingPosition`은 빈 픽업을 대기 위치로 복귀시킨다.
NG 운반 중처럼 캐리어를 잡고 있으면 기존 NG 이송 동작이 우선한다.

## 책임과 참조 방향

- `MainConveyor`: 메인 벨트, S1·S2 착좌·각 스테이션 지지 해제, SMEMA, 물류 우선순위와 S3 검사·착좌 요청.
- `PcbPlacer`, `BoltFasteningStation`, `InspectionStation`: 각 공정 실행과 현재 캐리어의 완료 판단.
- `InspectionStation`: 바코드·볼트 검사, 검사 후 복귀, S3 상승 전 픽업 위치 이동, NG 집기·운반.
...
검사 스테이션의 픽업 상승·미완료 인계 없음은 `InspectionStation.IsClear`에서 읽는다.
...
- Inspection과 NG Transfer는 하나의 Enabled를 사용한다. 비활성이면 S3 착좌 또는
  검사 위치(DOWN/스토퍼 UP/벨트 정지)에서 자기 루프가 완료 처리한다.
  검사를 건너뛴 캐리어는 NG 결과로 취급하지만 NG 이송도 비활성이므로 후방 SMEMA로 배출한다.
  비활성 갠트리의 대기 위치 이동·확인을 요구하지 않는다.
