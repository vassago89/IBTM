...
ADC 실제 체결은 START 전 이벤트 번호(3200)를 한 번 조회한 뒤 컨트롤러 알림을 기다린다. 체결 중 이벤트 번호·결과·RUN/READY의 반복 조회와 자동 재시도는 하지 않으며 조회 주기 설정도 없다.
요청에 속하지 않는 CRC 정상 `84 03`을 결과 알림으로 가정하는 실장비 확인용 동작이다. 제조사에서 완료 신호라고 확인된 의미는 아니며, 알림 자체를 OK/NG 결과로 기록하지 않는다.
수신은 `SerialPort.DataReceived`에서 프레임을 조립한다. 체결 쪽은 START 직전에 알림을 구독하고 `TaskCompletionSource`를 await한다. 별도 수신 루프·주기 타이머는 없으며 기존 `FasteningTimeoutMilliseconds`로 전체 대기를 제한한다.
같은 포트·슬레이브의 알림이 오면 전체 결과(3200~3213)를 한 번만 읽는다. 중복 알림도 재조회를 만들지 않는다. 요청 대기 중 도착한 `84 03`은 해당 요청의 거절 응답으로 유지하고 알림으로 다시 돌리지 않는다.
시작 전과 이벤트 번호가 다른지 비교하며 +1, 증가 방향 또는 대소관계를 가정하지 않는다. 번호가 같거나 방향·프리셋 변경 이벤트이면 새 체결 결과 미확보로 NG를 기록한다. 체결 OK/NG는 선택한 프리셋·체결 방향도 일치해야 기록한다.
START 전·종료 후 알림은 다음 볼트에 이월하지 않는다. 알림이 없으면 타임아웃이며, 1회 결과 조회의 거절·불일치 응답과 새 Error 결과는 해당 볼트의 NG와 오류 내용으로 기록한다.
...
ADC 체결 타임아웃은 포트·주소, 알림 수신 여부와 시작/마지막 이벤트·상태·프리셋을 표시한다.
