...
internal static class VirtualImageFactory
{
        ...
        IEnumerable<VirtualDataMatrix> dataMatrices)
    {
        ...
        var visibleBolts = boltCentres.Where(
            bolt =>
                Math.Abs(bolt.X - center.X) <= halfWidth + BoltRadius
                    && Math.Abs(bolt.Y - center.Y) <= halfHeight + BoltRadius)
            .ToArray();
        var pixels = new byte[Width * Height * ImageFrame.ColorChannelCount];
        for (var pixelY = 0; pixelY < Height; pixelY++)
        {
            for (var pixelX = 0; pixelX < Width; pixelX++)
            {
                var x = center.X + ((pixelX - (Width / 2)) * InspectionMillimetersPerPixel);
                var y = center.Y + ((pixelY - (Height / 2)) * InspectionMillimetersPerPixel);
                var color = InspectionColor(x, y, visibleBolts);
                SetPixel(pixels, pixelX, pixelY, color.Blue, color.Green, color.Red);
            }
        }
        ...
        foreach (var code in dataMatrices)
        {
            ...
            const int border = 8;
            var matrix = new DataMatrixWriter().encode(code.Text, BarcodeFormat.DATA_MATRIX, 0, 0);
            ...
            for (var y = Math.Max(0, -top); y < Math.Min(size, Height - top); y++)
                for (var x = Math.Max(0, -left); x < Math.Min(size, Width - left); x++)
                {
                    var black = x >= border
                        ...
                        && x < size - border
                        && y < size - border
                        && matrix[(x - border) * matrix.Width / (size - border * 2), (y - border) * matrix.Height / (size - border * 2)];
