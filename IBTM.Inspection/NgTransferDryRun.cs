...
public sealed class NgTransferDryRun : AutoUnit
{
    ...
    public NgTransferState State => _move.State(Destination, canPickUp: true);
    ...
    private Task ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        switch (state)
        {
            ...
            case NgTransferState.WaitingForCarrier when _move.CarrierPresent(Destination):
                break;
            default:
                return _move.ExecuteAsync(Destination, state, cancellationToken) ?? WaitForChangeAsync(cancellationToken);
        }
        Destination = NgCarrierMove.Opposite(Destination);
        NotifyChanged();
