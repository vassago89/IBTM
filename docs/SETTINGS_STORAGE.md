...
Teaching의 좌표 변경은 `Record Position`에서만 한다. 해당 포인트의 원본 좌표를 즉시 변경하며,
화면용 인계 좌표 복사본이나 Save 시 좌표 적용 과정은 없다.
Teaching의 저장 버튼은 `Save` 하나다. 기록된 양쪽 인계값과 현재 레시피를 그대로 저장한다.
인계 편집값과 편집 중인 레시피 이름은 티칭 화면을 나갔다 돌아와도 유지한다.
화면 재진입·목록 갱신·이동·Save에서 좌표를 덮어쓰지 않으며, 편집 중인 레시피 이름도 보존한다.
검사 영상과 촬영 XY도 `Record Position`으로 함께 기록한다. ROI 크기·해상도 보정은 좌표를 다시 계산하거나 초기화하지 않는다.
다른 레시피 불러오기와 New Recipe처럼 사용자가 내용을 교체하는 명령은 그대로 적용한다.
