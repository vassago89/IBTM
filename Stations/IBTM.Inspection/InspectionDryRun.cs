...
public sealed class InspectionDryRun : AutoUnit
{
    ...
    public bool Ready
    {
        get
        {
            return _work.CarrierSeated && _transfer.IsClear;
        }
    }
    ...
    public int? ActiveBolt
    {
        get
        {
            return _current?.Bolt?.Number;
        }
    }
    ...
    public InspectionDryRunState State
    {
        get
        {
            return !_work.CarrierPresent
                ? InspectionDryRunState.WaitingForCarrier
                : !_work.CarrierSeated
                    ? InspectionDryRunState.WaitingForSeat
                    : !_transfer.IsClear
                        ? InspectionDryRunState.WaitingForGantry
                        : _current is not { } target
                            ? InspectionDryRunState.Ready
                            : target.Bolt is { } invalidBolt
                                && !_inspector.HasPosition(invalidBolt)
                                || target.Bolt is null
                                && !_inspector.HasBarcodeRegion(target.Pcb)
                                ? InspectionDryRunState.Ready
                                : target.Bolt is { } bolt
                                    ? _inspector.IsAt(bolt)
                                        ? InspectionDryRunState.InspectingBolt
                                        : InspectionDryRunState.MovingToBolt
                                    : _inspector.IsAtBarcode(target.Pcb)
                                        ? InspectionDryRunState.ReadingBarcode
                                        : InspectionDryRunState.MovingToBarcode;
        }
    }
    ...
    public async Task RunAsync(CancellationToken cancellationToken)
    {
        ...
        var targets = Enum.GetValues<HeatSinkSlot>()
            ...
            .SelectMany(
                slot =>
                    new[] { new Target(slot, null) }.Concat(
                        pcb.GetBolts(slot).OrderBy(bolt => bolt.Number).Select(
                            bolt => new Target(slot, bolt))))
            .ToArray();
        if (targets.Length == 0)
        ...
        var next = targets.FirstOrDefault(
            target => target.Pcb == previous?.Pcb && target.Bolt?.Number == previous?.Bolt?.Number);
        if (next is null)
            Direction = InspectionRouteDirection.Forward;
        _targets = targets;
    ...
    private async Task ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        if (Direction == InspectionRouteDirection.Forward
            && index == _targets.Length - 1
            || Direction == InspectionRouteDirection.Return
            && index == 0)
        {
            CompletedPasses++;
            Direction = Direction == InspectionRouteDirection.Forward
                ? InspectionRouteDirection.Return
                : InspectionRouteDirection.Forward;
        }
