...
public enum NgConveyorDryRunState
{
    [Description("Load one carrier")]
    WaitingForCarrier,
    [Description("Raise the NG pickup")]
    WaitingForPickup,
    [Description("Lowering shuttle")]
    LoweringShuttle,
    [Description("Preparing stopper")]
    PreparingStopper,
    [Description("Moving to P1")]
    MovingToPosition1,
    [Description("Returning to P3")]
    ReturningToShuttle,
    [Description("Raising shuttle")]
    RaisingShuttle,
...
public sealed class NgConveyorDryRun : AutoUnit
{
    ...
    public NgConveyorDryRun(
        NgCarrierConveyor conveyor,
        NgShuttle shuttle,
        INgCarrierTransferFeedback pickup)
    {
        _conveyor = conveyor;
        _shuttle = shuttle;
        _pickup = pickup;
    ...
    public NgConveyorDryRunState State
    {
        ...
        {
            if (!Ready)
                return NgConveyorDryRunState.WaitingForPickup;
            if (Destination == NgConveyorDestination.None)
                return NgConveyorDryRunState.WaitingForCarrier;
            var arrived = Destination == NgConveyorDestination.Position1
                ? _conveyor.Position1Occupied
                : _shuttle.Feedback.CarrierDetected;
            if (arrived)
                return _shuttle.Feedback.Lift == NgShuttleLiftState.Up
                    ? NgConveyorDryRunState.Arrived
                    : NgConveyorDryRunState.RaisingShuttle;
            ...
            if (Destination == NgConveyorDestination.Position1)
                return !_conveyor.StopperUp
                    ? NgConveyorDryRunState.PreparingStopper
                    : NgConveyorDryRunState.MovingToPosition1;
            return !_conveyor.StopperDown
                ? NgConveyorDryRunState.PreparingStopper
    ...
    public async Task RunAsync(CancellationToken cancellationToken)
    {
        ...
        if (_conveyor.CarrierCount > 1
            || Destination == NgConveyorDestination.None
            && _conveyor.CarrierCount != 1)
            throw new InvalidOperationException("NG conveyor round trip requires exactly one carrier.");
        if (Destination == NgConveyorDestination.None)
            Destination = _conveyor.Position1Occupied
                ? NgConveyorDestination.Shuttle
                : NgConveyorDestination.Position1;
    ...
    private async Task ExecuteAsync(CancellationToken token)
    {
        switch (State)
        {
            case NgConveyorDryRunState.LoweringShuttle:
                await _shuttle.SetDownAsync(true, token);
                break;
            case NgConveyorDryRunState.PreparingStopper:
                await _conveyor.SetStopperUpAsync(Destination == NgConveyorDestination.Position1, token);
                break;
            case NgConveyorDryRunState.MovingToPosition1:
                await _conveyor.RunUntilAsync(InputIo.NgConveyorPosition1Occupied, true, false, token);
                break;
            case NgConveyorDryRunState.ReturningToShuttle:
                await _conveyor.RunUntilAsync(InputIo.NgShuttleCarrierDetected, true, true, token);
                break;
            case NgConveyorDryRunState.RaisingShuttle:
                await _shuttle.SetDownAsync(false, token);
                break;
