...
Settings → Camera & Lighting → Lighting → Stabilization Delay (ms)에서 조정하고 Save Settings로 저장한다. 자동 검사·티칭 Grab에 적용하며, 이미 조명이 켜진 Live 중 Grab은 그대로 촬영한다.
...
한쪽이 실패하거나 STOP되면 다른 쪽도 취소하고 이동 정지·발사 OFF 정리가 끝날 때까지 기다린다.
슈팅 진공 입력은 표시만 하며 공급 생략·도착·진공 해제·제거된 캐리어 결과 정리 조건으로 사용하지 않는다. 다른 헤드·이스케이프 신호가 바뀌어도 피더 준비 대기를 완료하지 않는다.
...
- AJIN 디바이스 동작은 `C:\git\AnyWave\AnyWave.Device\Motions\Ajin\AjinService.cs`가 기준이다.
  ...
  저장된 축 설정 비율로 현재 좌표를 다시 보정하지 않는다.
  Unit/Pulse와 가감속 단위는 `Initialize`에서만 적용·확인한다.
  이동·Jog·HOME·준비 상태 조회에서는 단위를 다시 읽거나 비교·설정하지 않는다.
  위치 명령은 `AxmMoveStartPos` / `AxmMoveStartMultiPos`로 시작한 뒤 실제 피드백으로 완료를 기다린다.
  이동 완료는 실행한 축의 정지·InPosition 신호로 판단하며, 목표·실제 좌표 차이에 고정 허용 오차를 추가 적용하지 않는다.
...
XY·체결 Z 이동 중 테이블 피드백이 달라지면 이동을 취소하고 정지 확인 후 오류를 표시한다.
계산된 볼트 위치에는 XYZ를 표시하고, 기준핀 XY·Z 전용 항목·피더 픽업은 각각의 기존 이동 순서를 유지한다.
`Record Position`의 장비값 자동 저장이 실패해도 기록한 좌표는 메모리에 유지한다.
...
정방향 입구 감지·목적지 HS2 도착·출구 감지/해제에는 `ConveyorSettings.TransferTimeoutSeconds`(기본 5초)를 적용한다.
...
완료된 작업의 이송 선택도 스토퍼 위치로 막지 않는다. 스토퍼 하강은 백업 플레이트 상승 완료 후와
...
ADC 실제 체결은 START 전 이벤트 번호(3200) 1개를 조회하고, 체결 중에도 같은 레지스터 1개만 주기적으로 조회한다. 마지막으로 수집한 이벤트와 번호가 달라졌을 때만 전체 결과(3200~3213)를 읽는다. 전체 조회가 거절되면 해당 이벤트를 수집한 것으로 기록하지 않고 다음 주기에 다시 조회한다.
시작 전과 이벤트 번호가 다른지 비교하며 +1, 증가 방향 또는 대소관계를 가정하지 않는다. 번호가 같거나 방향·프리셋 변경 이벤트이면 계속 기다린다. 체결 OK/NG는 선택한 프리셋·체결 방향도 일치해야 기록한다.
조회 간격은 `HantasSettings.ResultPollingIntervalMilliseconds`(기본 100ms, 양수)이며 Settings → Operation & Timing → Bolt Fastening · Timing에서 조정한다. 첫 조회 전 및 미완료 응답 후 다음 요청까지 이 시간만큼 기다리고, 요청은 중첩하지 않는다. 전체 체결 제한 시간은 기존 `FasteningTimeoutMilliseconds`를 사용한다.
수신은 `SerialPort.DataReceived`에서 프레임을 조립해 해당 요청에 전달한다. Auto Data Output 설정 없이 요청·응답으로 결과를 확인하며, 체결 대기 중 별도 RUN/READY 조회는 하지 않는다.
ADC 체결 중 결과 조회의 0x03 예외 응답과 요청에 맞지 않는 정상 RTU 응답은 기록하고, 설정된 조회 간격으로 기존 체결 제한 시간 안에서 조회를 이어간다. START를 다시 보내거나 제한 시간을 연장하지 않는다. START 전 기준 이벤트 조회에는 이 처리를 적용하지 않는다.
새 Error 결과와 나머지 조회 예외는 해당 볼트의 NG와 오류 내용으로 기록한다.
오류 응답으로 결과를 받지 못했으면 토크는 빈값으로 둔다. I/O START OFF 후 ADC 상태를 한 번 조회하고 결과를 넘긴다. 헤드를 올린 뒤 다음 볼트로 진행한다.
이전 볼트의 Error 이벤트를 새 볼트 결과로 재사용하지 않는다. 통신 단절·깨진 응답·START 실패·모터 정지 확인 실패와 다음 START의 실제 READY 조건은 유지한다.
체결 중에는 볼트 접촉으로 헤드가 끝까지 내려가지 않을 수 있으므로 DOWN 출력만 보내고 하강 입력을 기다리지 않는다. 픽업·슈팅, 통신형·IO형 모두 동일하다.
헤드 DOWN 출력 → 체결 결과 수신 → 진공 OFF(픽업만 입력 확인) → 헤드 UP 확인을 각각 로그로 남긴다. 체결 후 상승 및 이동 전 UP 피드백 확인은 유지한다.
ADC 체결 타임아웃은 포트·주소, 조회 간격과 시작/마지막 이벤트·상태·프리셋을 표시한다.
픽업 볼트는 각 위치에서 한 번만 체결한다. 체결기 준비 확인과 검사도 유지한다.
XY 이동에 필요한 양쪽 헤드 상승과 Safe Z도 유지한다.
