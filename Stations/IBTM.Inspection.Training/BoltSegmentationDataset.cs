...
internal sealed class BoltSegmentationDataset
{
    ...
    internal BoltSegmentationDataset(BoltTrainingStore store, CancellationToken cancellationToken)
    {
        var samples = store.GetSamples()
            .Where(sample => sample.Included && sample.Label != BoltLabel.Unlabeled)
            .ToArray();
        ...
        if (positive == 0)
            throw new InvalidDataException("Training polygons contain no bolt-recess pixels.");
        ...
        CancellationToken token)
    {
        var samples = store.GetSamples()
            .Where(
                sample =>
                    sample.Included
                        && sample.Label != BoltLabel.Unlabeled
                        && sample.Use == BoltSampleUse.Validation)
            .ToArray();
        if (samples.Length == 0)
    ...
    private static BoltTrainingInput[] Prepare(
        BoltSampleInfo[] samples,
        BoltTrainingStore store,
        CancellationToken token)
    {
