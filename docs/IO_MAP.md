...
피더 OFF 또는 Repeat 드라이런에서는 FASTEN 판정을 하지 않고, 헤드 하강 출력 후 설정 시간(기본 2초)이 지나면 START를 OFF한다.
동작 완료는 `DryRun`으로 기록하며 토크는 미측정(null)이다. 실제 체결 결과 대기 시간과 드라이런 시간은 별도 설정이다.
중단·타임아웃·I/O 오류 시 START를 끄고 미완료 상태를 남긴다. 임의로 재체결하지 않으며
작업자가 캐리어와 보유 부품을 수동 제거하고 RESET한 뒤 새 작업을 시작한다. 중단된 볼트는 재체결하지 않는다.
