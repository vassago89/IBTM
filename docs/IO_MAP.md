...
기존 DB는 위 항목의 종전 기본 주소 묶음과 일치할 때만 함께 이동한다.
일부라도 별도로 조정한 입력 묶음은 유지한다.
단일 출력 방식이 확인되었으므로 Supply IPM의 저장된 별도 후진 출력(OffNumber)은 제거한다.
출력 방향과 기존 접점 보정은 유지한다.
...
위 260913 주소 보정은 명시된 종전 기본 매핑에만 적용하는 예외다.
제거된 스테이션 전용 캐리어 입력 키는 시작 시 매핑에서 제외하고, 옛 버퍼 설정 섹션은 읽지 않는다.
그 밖의 지정된 과거 IO 변환은 [설정과 저장](SETTINGS_STORAGE.md)을 참고한다. 매핑 저장 후 재시작한다.
