...
시리얼 포트별 기존 `_exchange` 세마포어가 상태/결과/진단 요청 전체를 직렬화하고, 수신은 `SerialPort.DataReceived`에서 프레임을 조립한다. 모니터 시작/종료와 샘플 발행을 동기화하고 불변 샘플을 원자적으로 교체한다. 동일 포트의 생산/진단 헤드가 모니터를 중복 생성하지 않는다.
