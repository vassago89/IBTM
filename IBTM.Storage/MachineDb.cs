...
internal sealed class MachineDb : DbContext
{
    ...
    internal static DbContextOptions<MachineDb> CreateOptions(string path) =>
        new DbContextOptionsBuilder<MachineDb>().UseSqlite(
            new SqliteConnectionStringBuilder { DataSource = path }.ToString()).Options;
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        image.HasOne<RecipeRow>().WithMany().HasForeignKey(row => row.RecipeName)
