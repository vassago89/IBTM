...
{
    [Description("Raise and clear the working heads")]
    Unavailable,
    [Description("Load one carrier inside the machine")]
    WaitingForCarrier,
    [Description("Lowering plates and stoppers for return")]
    ClearingReturnPath,
    [Description("Returning to front sensor")]
    Returning,
    [Description("Preparing station")]
    Preparing,
    [Description("Moving to station")]
    Moving,
    [Description("Seating carrier")]
    Seating,
...
public sealed class MainConveyorDryRun : AutoUnit
{
    ...
    public bool ReturningToStation1
    {
        get
        {
            return _route == Route.ReturnToStation1 && !AtStation1;
        }
    }

    public bool AtStation1
    {
        get
        {
            return Destination == MainConveyorDestination.Station1
                && State == MainConveyorDryRunState.Arrived;
        }
    }

    private ConveyorStation Target
    {
        get
        {
            return _stations[(int)Destination - (int)MainConveyorDestination.Station1];
        }
    }

    private ConveyorStation? Source
    {
        get
        {
            return Destination > MainConveyorDestination.Station1
                ? _stations[(int)Destination - (int)MainConveyorDestination.Station1 - 1]
                : null;
        }
    }
    ...
    {
        return station.BackupPlate == StationCylinderState.Down
            && station.Stopper == StationCylinderState.Down;
    }

    public MainConveyorDryRunState State
    {
        get
        {
                ...
                _
                    ...
                    => Target.CarrierPresent
                        ? Target.BackupPlate == StationCylinderState.Up
                            ...
                            ? MainConveyorDryRunState.Arrived
                            : MainConveyorDryRunState.Seating
                        : Target.BackupPlate == StationCylinderState.Down
                            && Target.Stopper == StationCylinderState.Up
                            && (Source is null || Released(Source))
                            ? MainConveyorDryRunState.Moving
                            : MainConveyorDryRunState.Preparing,
            };
        }
    }
    ...
    {
        return RunAsync(Route.RoundTrip, cancellationToken);
    }

    public Task ReturnToStation1Async(CancellationToken cancellationToken)
    {
        return RunAsync(Route.ReturnToStation1, cancellationToken);
    }
    ...
    private async Task RunAsync(Route route, CancellationToken cancellationToken)
    {
        ...
        if (_route != route
            || Destination == MainConveyorDestination.None
            || route == Route.ReturnToStation1
            && index > 0)
        {
            ...
            Destination = route == Route.ReturnToStation1
                ...
                : index < 0
                    ? MainConveyorDestination.Station1
                    : MainConveyorDestination.Station1 + index;
            _route = route;
        }

        NotifyChanged();
        await _conveyor.RunDryRunAsync(
            token => RunLoopAsync(
                ExecuteAsync,
                token,
                route == Route.ReturnToStation1 ? () => AtStation1 : null),
            cancellationToken);
    ...
    private async Task ExecuteAsync(CancellationToken cancellationToken)
    {
        switch (State)
        {
            ...
            case MainConveyorDryRunState.Preparing:
                await Task.WhenAll(
                    Target.PrepareToReceiveAsync(cancellationToken),
            ...
            case MainConveyorDryRunState.Seating:
                await Task.WhenAll(
                    Target.SeatAsync(cancellationToken),
