...
public sealed class PcbBufferSettings : Setting
{
    ...
    public TeachingPosition[] GetTeachingPositions()
    {
        return [
            new(
                ...
                TeachMode.XOnly,
                () => new() { X = SupplyBoundary1 },
                p => SupplyBoundary1 = p.X,
                this) { Staged = true },
            new(
                TeachingTarget.SupplyBufferBoundary2,
                MotionGroup.PcbSupply,
                TeachMode.XOnly,
                () => new() { X = SupplyBoundary2 },
                p => SupplyBoundary2 = p.X,
                this) { Staged = true },
            new(
    ...
    {
        return x >= Math.Min(SupplyBoundary1, SupplyBoundary2)
