...
슈팅 포인트 이동(Safe Z → XY → 체결 Z)과 볼트 공급·도착 시간 대기를 병렬로 수행한다. 양쪽 헤드 UP·픽업 테이블 UP 상태에서 시작하고, 이동과 공급이 모두 끝난 뒤 튜브 해제·이스케이프 후진을 확인하고 체결한다.
...
ADC 체결 중 새 Error 결과 또는 결과 조회에 대한 정상 CRC의 오류 응답(예: 0x03)은 해당 볼트의 NG와 오류 내용으로 기록한다.
오류 응답으로 결과를 받지 못했으면 토크는 빈값으로 둔다. 모터 STOP → RUN OFF 확인 후 결과를 넘기고, 헤드를 올린 뒤 다음 볼트로 진행한다.
