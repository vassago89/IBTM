namespace IBTM.Domain.Configuration;
...
public sealed class MachineConfig
{
    ...
    {
        Zone1Ref = src.Zone1Ref.Clone();
        Zone2Ref = src.Zone2Ref.Clone();
        Zone3Ref = src.Zone3Ref.Clone();
        Offset3To1 = src.Offset3To1.Clone();
        Offset3To2 = src.Offset3To2.Clone();
        Zone1Motion = src.Zone1Motion.Clone();
        Zone2Motion = src.Zone2Motion.Clone();
        Zone3Motion = src.Zone3Motion.Clone();
    ...
    /// <summary>Zone 1 좌표를 Zone 3 좌표로 변환</summary>
    public AxisPos FromZone1ToZone3(AxisPos zone1Pos) => new()
    {
        X = zone1Pos.X + Offset3To1.X,
        Y = zone1Pos.Y + Offset3To1.Y,
        Z = zone1Pos.Z + Offset3To1.Z,
    };

    /// <summary>Zone 2 좌표를 Zone 3 좌표로 변환</summary>
    public AxisPos FromZone2ToZone3(AxisPos zone2Pos) => new()
    {
        X = zone2Pos.X + Offset3To2.X,
        Y = zone2Pos.Y + Offset3To2.Y,
        Z = zone2Pos.Z + Offset3To2.Z,
    };

    /// <summary>선택된 Zone의 모션 파라미터 반환</summary>
    public ZoneMotionParams GetMotionParams(int zone) =>
        new[] { Zone1Motion, Zone2Motion, Zone3Motion }[zone - 1];
}
...
public sealed class ZoneMotionParams
{
    ...
    public ZoneMotionParams Clone() => new()
    {
        SpeedXY = SpeedXY,
        SpeedZ = SpeedZ,
        Acceleration = Acceleration,
        Deceleration = Deceleration,
        SoftLimitXPlus = SoftLimitXPlus,
        SoftLimitXMinus = SoftLimitXMinus,
        SoftLimitYPlus = SoftLimitYPlus,
        SoftLimitYMinus = SoftLimitYMinus,
        SoftLimitZPlus = SoftLimitZPlus,
        SoftLimitZMinus = SoftLimitZMinus,
        HomeOffsetX = HomeOffsetX,
        HomeOffsetY = HomeOffsetY,
        HomeOffsetZ = HomeOffsetZ,
    };
